...
color Renderer::trace(const Ray& ray, const shared_ptr<Scene>& scene) {
	...
	bool isHit = scene->hit(ray, 0.001, infinity, rec);
	if (!isHit) {
		return background;
	}
	...
	case MatType::GLASS: {
		...
		//reflection
		vec3 reflectedDir = -2 * glm::dot(ray.dir, rec.normal) * rec.normal + ray.dir;
		Ray reflected(rec.p, glm::normalize(reflectedDir));
		color reflectColor = trace(reflected, scene);
		...
		if (!cannot_refract) {
			...
			color refractColor = trace(refracted, scene);
		...
		// shading 
	{
		for (auto& light : scene->lights) {
			...
			float ambient = ka ;
			float specular = std::powf(HoN,20.0f);

			finalColor += ambient * lightColor + (NoL + specular) * lightColor * albedo;
		}
	}
		break;
	case MatType::LIGHT:
...
void Renderer::render(const shared_ptr<Scene>& scene) {
	...
	for (int j = h-1; j >= 0; j--) {
		for (int i = 0; i < w; i++) {
			...
			image[3 * pos] = 255 * sqrtf(color.x);
			image[3 * pos + 1] = 255 * sqrtf(color.y);
			image[3 * pos + 2] = 255 * sqrtf(color.z);
		}
	}
}
